...
MICTLANX_RM_LOG_SHOW     = bool(int(os.environ.get("MICTLANX_RM_LOG_SHOW","0")))
...
async def get_stats_from_peers(peers:List[InterfaceX.Peer],pagination:Dict[str,Pagination]={}):
    for i,peer in enumerate(peers):
        ...
        if peer.peer_id in pagination:
            start,end, completed = pagination.get(peer.peer_id).next()
...
async def run_rm(
    ...
    # Init pagination
    paginations:Dict[str,Pagination] = dict([(k,Pagination(n = v, batch_size=params.batch_size)) for k,v in local_n_balls_map.items()])
    ...
    while True:
        ...
        try:
            ...
            remaining_balls_peers_ids = list(
                dict(
                    filter(lambda x: not x[1].next()[2], paginations.items())
                ).keys()
            )
